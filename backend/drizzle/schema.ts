...
export const lectureVideos = pgTable(
  'lecture_videos',
  {
...
export const questionChoices = pgTable(
  ...
  (table) => [
    uniqueIndex('idx_choices_context').using(
      'btree',
      table.questionId.asc().nullsLast().op('int8_ops'),
...
export const lectureVideoCompletions = pgTable(
  'lecture_video_completions',
  {
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' })
      .defaultNow()
      .notNull(),
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.studentId, table.videoId],
      name: 'lecture_videos_watched_pkey',
    }),
  ],
);
...
export const courseEnrollments = pgTable(
  'course_enrollments',
  {
    ...
    id: bigint({ mode: 'number' }).generatedByDefaultAsIdentity({
      name: 'course_enrollments_id_seq',
      startWith: 1,
      increment: 1,
      minValue: 1,
      maxValue: 9223372036854775807,
  ...
  (table) => [
    ...
    foreignKey({
      ...
      .onDelete('cascade'),
    primaryKey({
      columns: [table.studentId, table.courseId, table.id],
...
export const answerSubmissions = pgTable(
  'answer_submissions',
  {
    ...
    questionId: bigint('question_id', { mode: 'number' }).notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    examSubmissionId: bigint('exam_submission_id', {
      mode: 'number',
    }).notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
